...
def register_prompts(mcp: FastMCP) -> None:
    ...
    def policy_reception_review(policy_description: str, topic: str) -> str:
        ...
        return (
            ...
            f"see the lay of the land: how many contributions exist, house / section / "
            f"year breakdowns, top debates. (No party breakdown at this level — Hansard's "
            f"corpus search doesn't expose per-contribution party; by_party and "
            f"top_contributors are always empty here.) Read the counts as signals, not "
            f"as positions.\n\n"
            ...
            f"  (b) Cross-party engagement — infer from attributed_to prefixes and "
            f"party abbreviations you can see directly in the citation text (step 2); "
            f"resolve a specific member's authoritative party via parliament_find_member "
            f"if needed, don't guess it from the string\n"
