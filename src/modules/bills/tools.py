...
from .models import BillDetail, BillSponsor, BillStage, BillSummary
...
def _parse_bill_detail(data: dict) -> BillDetail:
    sponsors = []
...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def bills_search_bills(params: BillSearchInput, ctx: Context) -> str:
        """Search UK parliamentary bills by keyword, session, house, or legislative stage.

        Returns bill summaries including title, current stage, and whether it has become an Act.
        Use bills_get_bill with the bill ID for full detail including sponsors and stage history.

        Args:
            params (BillSearchInput): query, optional session/house/stage filters.

        Returns:
            str: JSON array of BillSummary objects.
        """
        try:
            client: httpx.AsyncClient = ctx.lifespan_context["http"]
            qp: dict = {"SearchTerm": params.query, "Take": 20}
            ...
            return json.dumps([b.model_dump(mode="json") for b in bills], indent=2)
    ...
    async def bills_get_bill(params: BillDetailInput, ctx: Context) -> str:
        ...
        Args:
            params (BillDetailInput): bill_id from bills_search_bills.

        Returns:
            str: JSON BillDetail object.
        """
        try:
            client: httpx.AsyncClient = ctx.lifespan_context["http"]
            resp = await client.get(f"{BILLS_BASE}/Bills/{params.bill_id}")
            resp.raise_for_status()
            detail = _parse_bill_detail(resp.json())
